...
const CustomerDetailsComponent: React.FC = () => {
  ...
  const initDOM = () => {
    ...
    const createIonItem = (
      ...
      var ionLabel = React.createElement(IonLabel, { key: elementKey }, [
        labelElement,
        contentElement,
      ]);
    ...
    if (customerInfo.hasOwnProperty("name")) {
      ...
      customerInfoArray.push(createIonItem("name", label, customerInfo.name));
    }

    if (customerInfo.hasOwnProperty("age")) {
      var label = "العمر";
      // var content = "23";
      customerInfoArray.push(createIonItem("age", label, customerInfo.age));
    }
